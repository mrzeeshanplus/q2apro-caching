...
define('QA_CACHING_EXPIRATION_TIME', 60*60*(int) qa_opt('qa_caching_expiration_time')); // cache expiration in hours
...
class qa_caching_main
{
   ...
   /**
	...
	{		
		if(!empty($this->questionid))
		{
			if($this->do_caching() && !$this->is_logged_in && !$this->check_cache()) 
			{
				...
				// write entire page html into the cache
				$this->write_cache();
			}
		}
		return;
	 ...
	 * according to questionid in cache folder
	 */
	public function clear_cache($questionid=null)
	{
		if(!empty($questionid))
		{
			// remove cache file (** actually we could write the new cache file here instead of only deleting it)
			$filepath = $this->get_filename($questionid, false);
			@unlink($filepath);
		}
		else
		{
			...
			@exec( $comm ); // '@' to ensure no warning, if exec not available then it follows through to recursive delete
	...
	/**
	 * Returns a unique filepath+filename to store the cache. Also creates subfolder.
	 * @return type
	 */
	private function get_filename($questionid, $createfolder=true)
	{
		// to not have thousands of files in one folder, which could slow down the performance, we use subfolders for each 1k
		$subfolder = '';
		...
		if(!empty($questionid))
		{
			...
			// subfolder name, round to closest thousand
			// $subfolder = round($questionid, -3).'/';
			$subfolder = 1000*floor($questionid/1000).'/';
		}
		else
		{
			// md5 of url
		...
		$folderpath = QA_CACHING_DIR.'/'.$subfolder;
	...
	{
		// memo: the minify plugin should always keep spaces within pre tags
		// require_once QA_PLUGIN_DIR.'q2apro-caching/minify/HTML.php';
		// return Minify_HTML::minify($html);
		
		require_once QA_PLUGIN_DIR.'q2apro-caching/minify/tiny-html-minifier.php';
